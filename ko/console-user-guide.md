...
- **채널**: GameTalk에서 유저 간 채팅을 가능하게 하는 채널 관리, 공지 메시지 발송 및 조회
...
채널 태그명은 알파벳 대/소문자와 숫자 그리고 허용되는 기호(-_:.)만 사용할 수 있습니다. 최대 20글자까지 입력할 수 있습니다.
...
#### (1) 발송 타입
- **전체발송**: 발송 타입이 전체 발송인 공지를 검색합니다.
- **채널 태그**: 공지의 발송 타입이 채널 태그이고 설정한 태그 검색 조건에 맞는 채널에 발송된 공지를 검색합니다.
- **채널명**: 공지의 발송 대상이 채널 명이고 채널명이 입력한 값을 포함하는 채널에 발송된 공지를 검색합니다.
#### (2) 내용
- **전체**: 공지 메시지, 공지 메모에 입력한 값이 하나라도 포함되는 공지를 검색합니다.
- **공지 메시지**: 공지 메시지에 입력한 값이 포함되는 공지를 검색합니다.
- **공지 메모**: 공지 메모에 입력한 값이 포함되는 공지를 검색합니다.

#### (3) 발송 시간
- 공지를 발송한 시간이 입력한 시간 범위에 포함되는 공지를 검색합니다.

#### (4) 공지 상태
- **대기 중**: 공지가 예약 시간이 되지 않아 대기 중인 상태입니다.
...
#### (3) 발송 시간
- **즉시 발송**: 공지를 즉시 발송합니다.
- **예약 발송**: 원하는 시간에 공지를 발송합니다. 시간을 선택하고, Time Zone을 선택하면 선택된 Time Zone의 선택한 시간에 공지를 발송합니다.
